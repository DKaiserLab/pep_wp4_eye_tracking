...
for i = 1:length(D)
    ...
    currentImageCategory = imageCategories{strcmp(imageCategories{:,1},D(i).annotation.filename),2};
    currentImageCategory = char(currentImageCategory);
    ...
    for j = 1:length(D(i).annotation.object)
        ...
        % Check if the object is member of background
    ...
    for k = 1:length(mask_keys)
        ...
        % Write object size
        idx = ObjSize.(currentImageCategory) +1;
        ObjSize.(currentImageCategory).object{k} = obj_name;
        ObjSize(i).propSize(k) = mean(mask, 'all');
    end

    % Save 
    save(fullfile(home_annotations, '..', '..', 'ObjectSizes.mat'), "ObjSize")
end
